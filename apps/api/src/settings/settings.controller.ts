...
export class SettingsController {
  ...
  async updateAi(@Body() body: unknown) {
    ...
    if (dto.apiKey !== undefined) await this.settings.set(SETTINGS_KEYS.aiApiKey, dto.apiKey, true);
    if (dto.baseUrl) await this.settings.set(SETTINGS_KEYS.aiBaseUrl, dto.baseUrl);
    if (dto.model) await this.settings.set(SETTINGS_KEYS.aiModel, dto.model);
    return { ok: true };
  }
}
